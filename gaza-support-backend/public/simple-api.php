...
// Database configuration - Using SQLite to match Laravel configuration
...
if ($method === 'GET' && empty($segments)) {
    ...
    $stmt = $pdo->prepare("INSERT INTO testimonials (name, country, email, content, rating, approved, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 0, datetime('now'), datetime('now'))");
    $stmt->execute([
        $input['name'],
        $input['country'],
